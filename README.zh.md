...
### 2.4 网页调试模式（`npm run web`）

如果不想打包安装包，现场调试时也可以直接在浏览器里跑：
...
这会在本机启动一个**只监听 127.0.0.1** 的调试服务（随机端口 + 随机会话令牌），并自动打开浏览器，地址类似 `http://127.0.0.1:8765/?token=...`。终端窗口要保持打开——按 Ctrl+C 关闭服务时会自动断开所有已连接的设备。网页版和 Electron 客户端功能完全一致（设备总览/调试工作台/设备调试/通信日志/类型实例管理），也包括 RTU 串口调试（见 4.4）。

可用的环境变量：`MODBUSMATE_WEB_PORT`（固定端口，不用随机）、`MODBUSMATE_DATA_DIR`（`config.json` 存放位置）。加 `--no-open` 参数可以不自动弹出浏览器（自动化测试用）。

> 该服务只监听本机地址，且必须带上其自身输出的 token 才能访问，局域网内其他设备无法访问。
...
### 4.4 Modbus-RTU（USB/RS485）设备

ModbusMate 也支持通过 USB 转 RS485 转换器连接 Modbus-RTU 设备，Electron 客户端和网页调试模式（`npm run web`）都可以用。**一个串口只能独占连接一个从站**。

**连接步骤：**

1. 插上 USB-RS485 转换器，在连接栏把「通信方式」下拉框切到 **RTU（USB/RS485）**（调试工作台连接栏和设备实例弹窗里都有这个选项）。
2. 点击 **「刷新串口」**，从列表中选择对应端口：
   - **macOS**：一般显示为 `/dev/tty.usbserial-*` 或 `/dev/tty.usbmodem-*`（具体名字跟转换器芯片有关，常见的有 FTDI、CH340、CP210x）
   - **Windows**：显示为 `COM3`、`COM4` 等；如果列表里找不到，去「设备管理器 → 端口 (COM 和 LPT)」确认是否需要安装转换器驱动
3. 按设备的通信协议设置串口参数，常见默认值：**9600 波特率、8 数据位、无校验、1 停止位（即 9600 8N1）**，从站 ID `1`，超时 `2000ms`。
4. 点击 **「连接」**。

**接线提示：** 把转换器的 `A`/`B`（或 `D+`/`D-`）端子接到设备对应端子上——**A/B 接反是"其他参数都对但就是无响应"最常见的原因**。

**连不上怎么办（RTU）**

| 现象 | 可能的原因 |
|------|------------|
| 串口权限不足 | 其他程序（或者上一次没正常关闭的 ModbusMate）占用了串口；macOS 上可能需要给终端/应用授予 USB 访问权限 |
| 串口被占用 | 关闭同时在用这个串口的其他工具（如串口调试助手、PLC 组态软件）后再刷新 |
| 拔插后找不到端口 | 端口列表是刷新时的快照，重新插拔转换器后要再点一次「刷新串口」 |
| 通信超时 | 检查 A/B 接线、波特率/校验位/停止位是否和设备一致，以及从站 ID 是否正确 |
